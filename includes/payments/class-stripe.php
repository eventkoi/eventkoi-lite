...
class Stripe {
	...
	public function create_checkout_session( $args ) {
		...
		$idempotency_key = 'ek_checkout_' . ( $args['client_reference_id'] ?? '' ) . '_' . ( $args['metadata']['ticket_id'] ?? '' ) . '_' . ( $args['metadata']['quantity'] ?? '' );
	...
	public function retrieve_checkout_session( $checkout_id = null, $expand = array() ) {
		if ( empty( $checkout_id ) || empty( $this->stripe ) ) {
			return null;
		}
	...
	public function create_customer( $args ) {
		$idempotency_key = 'ek_customer_' . ( $args['email'] ?? '' );
		...
		try {
			...
			return new \WP_Error( 'eventkoi_stripe_invalid_auth', __( 'Invalid Stripe authentication.', 'eventkoi-lite' ) );
		}
	...
	public function get_or_create_customer() {
		...
		$query = DB::table( 'eventkoi_customers' )
			->select( 'customer_id' )
		...
		$customer = $this->stripe->customers->create(
			...
			array(
				'idempotency_key' => 'ek_customer_' . $current_user->user_email,
			)
		);

		DB::table( 'eventkoi_customers' )->upsert(
			array(
				'user_id'     => $user_id,
	...
	public function create_order( $args ) {
		...
		DB::table( 'eventkoi_orders' )->upsert( $order_args, array( 'checkout_id' ) );

		// Retrieve the inserted or existing order ID.
		$order_row = DB::table( 'eventkoi_orders' )
		...
		if ( ! empty( $charge ) && ! empty( $charge->id ) ) {
			...
			DB::table( 'eventkoi_charges' )->upsert(
				$charge_args,
				array( 'charge_id' )
			);
		}
		...
		if ( ! empty( $charge ) && ! empty( $charge->id ) ) {
			...
			switch ( $order_args['status'] ) {
				...
				case 'failed':
					$reason = isset( $charge->failure_message ) ? $charge->failure_message : __( 'Unknown reason', 'eventkoi-lite' );
	...
	/**
	 * Prepare order arguments for the eventkoi_orders table.
	 *
	...
	/**
	 * Prepare charge arguments for the eventkoi_charges table.
	 *
	 * @param array                 $args           Session args from Stripe webhook.
	 * @param \Stripe\PaymentIntent $payment_intent Stripe payment intent object.
	...
	/**
	 * Prepare customer arguments for the eventkoi_customers table.
	 *
	 * @param \Stripe\Customer $customer Stripe customer object.
	...
	public function refund( $charge_id = '', $checkout_id = '', $amount = 0 ) {
		if ( empty( $charge_id ) && empty( $checkout_id ) ) {
			return new \WP_Error( 'eventkoi_refund_missing_id', __( 'A charge ID or checkout ID is required.', 'eventkoi-lite' ) );
		}

		// If only checkout_id is provided, fetch the charge_id.
		if ( empty( $charge_id ) && ! empty( $checkout_id ) ) {
			$charge_id = DB::table( 'eventkoi_charges' )
				->select( 'charge_id' )
				->where( 'checkout_id', $checkout_id )
				->value( 'charge_id' );
		}
		...
		if ( empty( $charge_id ) ) {
			return new \WP_Error( 'eventkoi_refund_charge_not_found', __( 'Could not resolve charge ID from checkout session.', 'eventkoi-lite' ) );
		}
		...
		try {
			$refund = $this->stripe->refunds->create(
				$refund_args,
				array(
					'idempotency_key' => 'ek_refund_' . $charge_id . '_' . $amount,
				)
			);
		} catch ( \Stripe\Exception\ApiErrorException $e ) {
			return new \WP_Error( 'eventkoi_refund_stripe_error', $e->getMessage() );
		}
		...
		if ( ! empty( $refund ) && ! empty( $refund->payment_intent ) ) {
			...
			$payment_intent = DB::table( 'eventkoi_charges' )
				->select( 'payment_id' )
		...
		if ( empty( $payment_id ) ) {
			return new \WP_Error( 'eventkoi_refund_no_payment_intent', __( 'Could not resolve payment intent for this charge.', 'eventkoi-lite' ) );
		}
		...
		if ( empty( $payment->latest_charge ) ) {
			return new \WP_Error( 'eventkoi_refund_no_charge', __( 'Payment intent has no charge data.', 'eventkoi-lite' ) );
		}
		...
		// Sync refund status to local ticket_orders table for QR check-in and attendance data.
		if ( '' !== $status ) {
			$refund_items = array();

			// Fetch ticket_id and quantity from the local orders table for restock.
			$local_order = DB::table( 'eventkoi_orders' )
				->select( 'ticket_id', 'quantity' )
				->where( 'charge_id', $charge->id )
				->get();

			if ( ! empty( $local_order ) && ! empty( $local_order->ticket_id ) ) {
				$refund_items[] = array(
					'ticket_id'       => absint( $local_order->ticket_id ),
					'quantity'        => absint( $local_order->quantity ?? 1 ),
					'restock_tickets' => ( 'refunded' === $status ),
				);
			}

			\EventKoi\Core\Ticket_Order_Sync::sync_refund_to_local(
				'',
				$status,
				$args['amount_refunded'],
				$refund_items,
				$charge->id
			);
		}
	...
	public function verify_integration( $public_key = '' ) {
		if ( empty( $public_key ) ) {
			return new \WP_Error( 'eventkoi_stripe_missing_key', __( 'Stripe public key is required.', 'eventkoi-lite' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error( 'eventkoi_stripe_request_error', __( 'Stripe API request failed.', 'eventkoi-lite' ), $response->get_error_message() );
		}

		$body = json_decode( wp_remote_retrieve_body( $response ), true );

		if ( isset( $body['error']['message'] ) && str_contains( $body['error']['message'], 'Invalid API Key' ) ) {
			return new \WP_Error( 'eventkoi_stripe_invalid_key', __( 'Invalid API key provided.', 'eventkoi-lite' ) );
		}
			...
			try {
				$webhook = $this->stripe->webhookEndpoints->create(
					array(
						...
						'description'    => __( 'Stripe event listener for EventKoi.', 'eventkoi-lite' ),
						'url'            => $webhook_url,
					),
					array(
						'idempotency_key' => 'ek_webhook_' . md5( $webhook_url ),
					)
				);
