...
class Stripe {
	...
	public static function get_stripe_event() {
		...
		// Invalidate order caches so the next admin load fetches fresh data.
		Tickets::invalidate_order_caches();
	...
	public static function manual_stripe_session( WP_REST_Request $request ) {
		...
		if ( empty( $stripe_id ) ) {
			return new \WP_Error(
				'eventkoi_stripe_missing_session',
				__( 'Stripe session ID is missing.', 'eventkoi-lite' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( empty( $session ) ) {
			return new \WP_Error(
				'eventkoi_stripe_session_error',
				__( 'Unable to retrieve the Stripe session. Check that your secret key is set.', 'eventkoi-lite' ),
				array( 'status' => 400 )
			);
		}
	...
	public static function test_checkout_session() {
		...
		if ( empty( $secret_key ) ) {
			return new \WP_Error(
				'eventkoi_stripe_missing_key',
				__( 'Stripe secret key is missing.', 'eventkoi-lite' ),
				array( 'status' => 400 )
			);
		}
		...
		$return_url  = admin_url( 'admin.php?page=eventkoi#/settings/payments?stripe_session_id={CHECKOUT_SESSION_ID}' );
				...
				array(
					'price_data' => array(
						...
						'product_data' => array(
							...
							'name' => sprintf( __( 'Test payment for %d tickets', 'eventkoi-lite' ), $quantity ),
						),
						'currency'     => $currency,
					),
					'quantity'   => $quantity,
