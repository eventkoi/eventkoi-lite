...
export async function fetchPluginConfig() {
  ...
  // Use config inlined by PHP to avoid an extra network round-trip.
  if (eventkoi_params?.supabase_config) {
    globalThis.__eventkoi_config = eventkoi_params.supabase_config;
    return globalThis.__eventkoi_config;
  }
