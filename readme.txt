...
= 1.4.4.1 – Maintenance – 2026-09-15 =
* Maintenance: Version alignment with EventKoi Pro. No changes to EventKoi Lite in this release.

= 1.4.4.0 – Calendar start-hour reliability – 2026-09-14 =
* Fix: On some themes and optimisation setups, the week calendar could settle one hour before the configured start time. It now re-anchors reliably to your start hour even when the layout finishes loading late.
...
* Fix: Event popovers could extend past the calendar and get cut off, especially with an image and a longer description. They now stay fully visible inside the calendar.
* Improvement: The timezone selector now sits above the calendar toolbar, keeping the header compact.

= 1.4.1.0 – Plugin fields panel visibility fix – 2026-09-03 =
* Fix: Field panels from other plugins could vanish for a user who had once hidden them through Screen Options, with no way to bring them back. The fields panel now shows every plugin metabox.
...
* New: Show a taxonomy's assigned terms anywhere event data is used, through the event_tax_ block attribute or the [eventkoi] shortcode. The event editor shows both alongside the other fields.
* New: Put event dates into your SEO titles and descriptions. Rank Math and Yoast now offer %eventkoi_datetime%, %eventkoi_date%, %eventkoi_time%, %eventkoi_datetime_with_summary%, %eventkoi_location%, %eventkoi_timezone% and %eventkoi_recurrence%.
* New: Pick any Elementor template for an event, and open the event straight in Elementor from the event editor.
* New: A public query API for other plugins and themes to list events and their occurrences.
* Fix: A site whose EventKoi database tables had been dropped or emptied showed a white screen on every page. The missing table is now rebuilt and the page still renders.
* Fix: Subscribed calendar feeds sent monthly rules as a plain monthly repeat, so an event set to a weekday of the month, such as the second Friday, appeared on the wrong date in Apple Calendar and Google Calendar.
* Fix: Yearly rules exported the wrong months to subscribed calendars, and dropped January entirely.
* Fix: On a fresh install the plugin's own API key did not authenticate its requests, so parts of the admin returned an error until the key was regenerated by hand.
* Fix: An event tagged with a term beyond the first 500 in a large taxonomy lost that term when the event was opened and saved.
* Fix: Duplicating an event left the excerpt behind.
* Fix: Editing a recurring series' times discarded any changes made to individual occurrences.
* Fix: Fields that other plugins place above the event content, such as ACF's "High (after title)" groups, were hidden in the event editor.
* Fix: The calendar toolbar wrapped onto a second row in narrow containers, and the selected row count in admin tables was untranslated.
* Fix: The event editor tabs scrolled out of view on long events.
* Fix: Saving an event with an empty name gave no warning.
* Fix: On calendar events shorter than an hour the title sat outside the card.
* Fix: Expanded rows in event listings showed the series' first day instead of their own.
...
* Fix: Search now also finds events that have already taken place, sorted after upcoming ones.
* Fix: The search results list scrolls on its own without dragging the page, and closes on Escape or a click outside.
